...
pub fn evaluate(
    ...
    // Parse diff conditions: diff(12h)>N or diff(12h)<N
    if let Some(rest) = condition.strip_prefix("diff(") {
        return evaluate_diff(rest, value, past_value);
    }
...
fn evaluate_diff(rest: &str, value: Option<&str>, past_value: Option<&str>) -> EndpointState {
...
/// Parse the hours from a diff() condition, e.g., "diff(12h)>100" returns Some(12.0)
pub fn parse_diff_hours(condition: &str) -> Option<f64> {
    let rest = condition.strip_prefix("diff(")?;
    let close = rest.find(')')?;
