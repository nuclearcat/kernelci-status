...
pub async fn dashboard(
    ...
    let critical_count = endpoints.iter().filter(|e| e.state == "CRITICAL").count();
    let warning_count = endpoints.iter().filter(|e| e.state == "WARNING").count();
    let ok_count = endpoints.iter().filter(|e| e.state == "OK").count();
    let nodata_count = endpoints.iter().filter(|e| e.state == "NO_DATA").count();
